...
pred attacker_action[s, s' : State] {
  ...
  m.source.roles in Cardiologist)

  s'.icd_mode = s.icd_mode and
  s'.joules_to_deliver = s.joules_to_deliver and
  s'.impulse_mode = s.impulse_mode and
  s'.authorised_card = s.authorised_card and
  s'.last_action in AttackerAction
}
...
check no_attacker for 5 expect 0
...
// This assertion specifies that in all states, the last action of all next states that
// is not by an attacker, implies that the current state's last action
// is a RecvChangeSettings, which implies that it is not done by
// a Patient. That is, a Patient cannot change ICD settings, 
...
// The unexplained_assertion does not hold as the Principal has a set of roles, which can 
// hold either Cardiologist, Patient or both.
...
assert turns_on_safe {
  all s : State | s.last_action in RecvModeOn => ord/prev[s].last_action in SendModeOn
}
...
assert safe_messages {
  all s : State | one s.network => (s.network = ModeOnMessage || s.network = ChangeSettingsMessage)
}

check safe_messages for 2 expect 0
